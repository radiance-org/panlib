...
newScanBar(const ImgColorSpace *dcsp, const ImgStruct *simp, float r)
{
	...
	if (ImgPixelLen[dcsp->format] != 1) {
		DMESG(DMCparameter, "Destination image must be single-channel");
		return NULL;
	}
...
PdilateImage(ImgStruct *ib, const ImgStruct *ia, float rad)
{
	...
	if (!inStack++) {
		sprintf(dmessage_buf, "%s %s image by %.1f pixels",
				rad>0 ? "Dilating" : "Eroding",
				PdescribeCS(ia->csp, NULL), fabs(rad));
		DMESG(DMCtrace, dmessage_buf);
	}
						/* color/buffered output? */
	if (ImgPixelLen[ib->csp->format] != 1 || PimagesOverlap(ib, ia)) {
		ImgStruct	oimg;
		int		c;
		oimg = *ia;
		oimg.img = NULL;		/* check multi-channel case */
		if (ImgPixelLen[ia->csp->format] != 1) {
	...
	sbp = newScanBar(ib->csp, ia, rad);	/* 1-channel destination */
	if (sbp == NULL || !PnewImage(ib, .0)) {
		--inStack;
		return false;
	}
	for (y = 0; y < ia->yres; y++)
