...
static const float	YC709[3] = {0.21264f, 0.71517f, 0.07219f};
...
initBRIEF()
{
	...
	while (i--) {
	    float	crx = (float)drand48();
	    ubrief[i].c = 0;			// choose color channel
	    while ((crx -= YC709[ubrief[i].c]) > 0)
	    	ubrief[i].c++;
	    do {				// choose position pair
...
derivatImage(PanImage *ib, const PanImage &im, int dx, int dy)
{
	...
	for (y = dy; y < im.Height()-dy; y++) {
		const float	*ip0 = (const float *)im[y-dy] + dx;
		const float	*ip1 = (const float *)im[y+dy] + dx;
		float		*dp = (float *)(*ib)[y + dy/2] + dx/2;
...
PnormEdges(PanImage *imp, const PanImage &im, float blur)
{
	...
	for (int y = im.Height(); y--; ) {
		...
		for (int n = im.Width(); n--; ergb += 3, drgb += 3) {
			float	gf = YC709[0]*ergb[0] + YC709[1]*ergb[1]
						+ YC709[2]*ergb[2];
			int	i = 3;
			if (gf <= 5e-6f) {	// near black pixel?
				while (i--)
					drgb[i] = float(drgb[i] > 5e-6f)/(drgb[i] + 1e-7f);
				continue;
			}
			gf *= 0.1f;
			int	bestc = 0;
			...
			for (i = 3; i--; )
				drgb[i] = float(i==bestc)/(YC709[i]*drgb[i] + 1e-7f);
		}
	}
	eroIm.Init();
	dilIm ^= blur;			// blurred channel coefficients * orig
	dilIm *= im;
	return imp->Load(dilIm);	// combines to optimized edge channel
}
...
int
PanFeatureList::SetFeatures(const PanImage &im, ABitMap2 fmap, float blur)
{
	Clear();			// starting fresh
	if (im.Ready() < PICread)
		return -1;
	if ((im.Width() != fmap.Width()) | (im.Height() != fmap.Height())) {
		DMESG(DMCparameter, "Image and bitmap dimension mismatch");
		return -1;
	}
					// avoid positions in margins
	xres = im.Width();
	yres = im.Height();
	fmap.ClearRect(0, 0, xres, BRIEF_DIA/2);
	fmap.ClearRect(0, yres-BRIEF_DIA/2, xres, BRIEF_DIA/2);
	fmap.ClearRect(0, BRIEF_DIA/2, BRIEF_DIA/2, yres-BRIEF_DIA);
	fmap.ClearRect(xres-BRIEF_DIA/2, BRIEF_DIA/2, BRIEF_DIA/2, yres-BRIEF_DIA);
					// count up what's left
	if (!(nfeats = fmap.SumTotal()))
		return 0;
	PanImage	imRGB;		// get RGB version of image
	if (blur > 0.5f || !PmatchColorSpace(im.GetCS(), &ICS_RGB709, PICMall)
			|| !imRGB.LinkRO(im)) {
		imRGB.Init(ICS_RGB709);
		if (!imRGB.Load(im))
			return -1;
	}
	...
	for (n = x = y = 0; fmap.Find(&x, &y); x++, n++) {
		feat[n].x = x; feat[n].y = y;
		feat[n].descr.NewBitMap(BRIEF_BITS);
		for (int i = BRIEF_BITS; i--; ) {
			const BriefSamp	&bs = ubrief[i];
			if (imRGB.GetRowFloat(y+bs.yo1)[3*(x+bs.xo1)+bs.c] <
					imRGB.GetRowFloat(y+bs.yo2)[3*(x+bs.xo2)+bs.c])
				feat[n].descr.Set(i);
		}
	}
	return nfeats;
